...
function App(params) {
  ...
  this.config = require(params.dir + '/../.env');                  // Load the environment's config
  this.server = this._create_server(params.dir + '/helpers');   // Create the express server & helpers
  this.models = load(params.dir + '/models', [this.server, this.config]);                       // Load all models
  console.log("MODELS:", this.models);
  this.controllers = load(params.dir + '/controllers', [this.server, this.config]);             // Load all controllers
