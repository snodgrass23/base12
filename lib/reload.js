...
exports = module.exports = function(options){
  options = options || {
    dir:__dirname+'/app',       // base directory to watch
    ignore: ['public']          // ignore changes in these directories
  };
  ...
  function traverse(file) {
    if (file.indexOf('/') == -1) file = options.dir + '/' + file;
    fs.stat(file, function(err, stat){
      if (!err) {
        if (stat.isDirectory()) {
          if (options.ignore.indexOf(basename(file)) > -1) {
            return;
          }
